...
#define POOL_PROTO_CLI_RPC_LIST(ver)                                                               \
	...
	X(POOL_CONNECT, 0, ver >= 7 ? &CQF_pool_connect : &CQF_pool_connect_v6,                    \
	  ver >= 7 ? ds_pool_connect_handler_v7 : ds_pool_connect_handler_v6, NULL)                \
	X(POOL_DISCONNECT, 0, &CQF_pool_disconnect,                                                \
	  ver >= 7 ? ds_pool_disconnect_handler_v7 : ds_pool_disconnect_handler_v6, NULL)          \
	X(POOL_QUERY, 0, ver >= 7 ? &CQF_pool_query : &CQF_pool_query_v6,                          \
	  ver >= 7 ? ds_pool_query_handler_v7 : ds_pool_query_handler_v6, NULL)                    \
	X(POOL_QUERY_INFO, 0, ver >= 7 ? &CQF_pool_query_info : &CQF_pool_query_info_v6,           \
	  ver >= 7 ? ds_pool_query_info_handler_v7 : ds_pool_query_info_handler_v6, NULL)          \
	X(POOL_EXCLUDE, 0, &CQF_pool_exclude,                                                      \
	  ver >= 7 ? ds_pool_update_handler_v7 : ds_pool_update_handler_v6, NULL)                  \
	X(POOL_DRAIN, 0, &CQF_pool_drain,                                                          \
	  ver >= 7 ? ds_pool_update_handler_v7 : ds_pool_update_handler_v6, NULL)                  \
	X(POOL_EXTEND, 0, &CQF_pool_extend, ds_pool_extend_handler, NULL)                          \
	X(POOL_EVICT, 0, &CQF_pool_evict, ds_pool_evict_handler, NULL)                             \
	X(POOL_REINT, 0, &CQF_pool_add,                                                            \
	  ver >= 7 ? ds_pool_update_handler_v7 : ds_pool_update_handler_v6, NULL)                  \
	X(POOL_ADD_IN, 0, &CQF_pool_add_in,                                                        \
	  ver >= 7 ? ds_pool_update_handler_v7 : ds_pool_update_handler_v6, NULL)                  \
	X(POOL_EXCLUDE_OUT, 0, &CQF_pool_exclude_out,                                              \
	  ver >= 7 ? ds_pool_update_handler_v7 : ds_pool_update_handler_v6, NULL)                  \
	X(POOL_SVC_STOP, 0, &CQF_pool_svc_stop,                                                    \
	  ver >= 7 ? ds_pool_svc_stop_handler_v7 : ds_pool_svc_stop_handler_v6, NULL)              \
	X(POOL_ATTR_LIST, 0, &CQF_pool_attr_list,                                                  \
	  ver >= 7 ? ds_pool_attr_list_handler_v7 : ds_pool_attr_list_handler_v6, NULL)            \
	X(POOL_ATTR_GET, 0, &CQF_pool_attr_get,                                                    \
	  ver >= 7 ? ds_pool_attr_get_handler_v7 : ds_pool_attr_get_handler_v6, NULL)              \
	X(POOL_ATTR_SET, 0, &CQF_pool_attr_set,                                                    \
	  ver >= 7 ? ds_pool_attr_set_handler_v7 : ds_pool_attr_set_handler_v6, NULL)              \
	X(POOL_ATTR_DEL, 0, &CQF_pool_attr_del,                                                    \
	  ver >= 7 ? ds_pool_attr_del_handler_v7 : ds_pool_attr_del_handler_v6, NULL)              \
	...
	X(POOL_REPLICAS_REMOVE, 0, &CQF_pool_replicas_remove, ds_pool_replicas_update_handler,     \
	  NULL)                                                                                    \
	X(POOL_LIST_CONT, 0, &CQF_pool_list_cont,                                                  \
	  ver >= 7 ? ds_pool_list_cont_handler_v7 : ds_pool_list_cont_handler_v6, NULL)            \
	X(POOL_TGT_QUERY_MAP, 0, &CQF_pool_tgt_query_map, ds_pool_tgt_query_map_handler, NULL)     \
	X(POOL_FILTER_CONT, 0, &CQF_pool_filter_cont,                                              \
	  ver >= 7 ? ds_pool_filter_cont_handler_v7 : ds_pool_filter_cont_handler_v6, NULL)        \
...
#define POOL_PROTO_SRV_RPC_LIST                                                                    \
	...
	X(POOL_ADD_TGT, 0, &CQF_pool_add, ds_pool_update_handler_v6, NULL)                         \
...
#define DAOS_OSEQ_POOL_CONNECT_V6		/* output fields */		 \
	((struct pool_op_out)		(pco_op)		CRT_VAR) \
	((struct daos_pool_space_v6)	(pco_space)		CRT_RAW) \
	((struct daos_rebuild_status)	(pco_rebuild_st)	CRT_RAW) \
	/* only set on -DER_TRUNC */					 \
	((uint32_t)			(pco_map_buf_size)	CRT_VAR)
...
CRT_RPC_DECLARE(pool_connect_v6, DAOS_ISEQ_POOL_CONNECT, DAOS_OSEQ_POOL_CONNECT_V6)
...
#define DAOS_OSEQ_POOL_QUERY_V6		/* output fields */			 \
	((struct pool_op_out)		(pqo_op)			CRT_VAR) \
	((daos_prop_t)			(pqo_prop)			CRT_PTR) \
	((struct daos_pool_space_v6)	(pqo_space)			CRT_RAW) \
	((struct daos_rebuild_status)	(pqo_rebuild_st)		CRT_RAW) \
	/* only set on -DER_TRUNC */						 \
	((uint32_t)			(pqo_map_buf_size)		CRT_VAR) \
	((uint32_t)			(pqo_pool_layout_ver)		CRT_VAR) \
...
#define DAOS_OSEQ_POOL_QUERY_INFO_V6	/* output fields */		 \
	...
	((d_rank_t)			(pqio_rank)		CRT_VAR) \
	((uint32_t)			(pqio_tgt)		CRT_VAR) \
	((struct daos_space_v6)		(pqio_space)		CRT_RAW) \
	((daos_target_state_t)		(pqio_state)		CRT_VAR)
